...
class Oauth {
    ...
    public function makeRequest($url, $method = 'GET', Array $parameters = array(), $returnType = 'json', $includeCallback = false, $includeVerifier = false){
        ...
        print "make request<br>";
        print "url: $url<br>";
        print "parameters:<br><pre>";
        print_r($parameters);
        print "</pre>";
        ...
        } else {    // add access token to parameter list for oauth 2.0 requests
 ...
 public function validateAccessToken(){
    ...
    print "validate access token<br>";
    ...
    if(isset($_SESSION[$this->_prefix]['access_token']) || (isset($this->_access_token) && strlen($this->_access_token) > 0)){
        ...
        print "access token is found<br>";
        //exit;
    ...
    if(!isset($this->_access_token) || strlen($this->_access_token) == 0){
        ...
        print "no access token is found<br>";
        
        // handle oauth 1.0 flow
        if(isset($this->_request_token_url) && strlen($this->_request_token_url) > 0){
            
            print "handle oauth 1.0 flow<br>";
            
            // request token and authorize app
            if(!isset($_GET['oauth_token']) && !isset($_GET['oauth_verifier'])){
                
                print "request token and authorize app<br>";
                //exit;
            ...
            } else {  // request access token
                
                print "request access token<br>";
                
                print "oauth_token: " . $_GET['oauth_token'] . "<br>";
                print "_prefix: " . $this->_prefix . "<br>";
                print "session<br><pre>";
                print_r($_SESSION[$this->_prefix]);
                print "</pre><br>";
        ...
        if(isset($_GET['state']) && isset($_SESSION[$this->_prefix]['state']) && $_GET['state'] != $_SESSION[$this->_prefix]['state']){
        try {
...
protected function requestToken($returnType = 'flat', Array $values = array('oauth_token', 'oauth_token_secret')){
    ...
    protected function requestAccessToken($method = 'GET', Array $params = array(), $returnType = 'flat', Array $values = array('access_token', 'expires')){
        ...
        } else {  // set parameters for oauth 2.0 request        
    ...
    protected function authorize(/*Array*/ $scope = array(), $scope_seperator = ',', $attach = null){
        ...
        } else {  // build authorize url for oauth 2.0 requests
        ...
        print "_authorize_url: " . $this->_authorize_url ."<br>";
        //exit;
 ...
 protected function getParameters($response, $returnType){
  if($returnType != 'json'){
   $r = explode('&', $response);
   $params = array();
   foreach($r as $v){
